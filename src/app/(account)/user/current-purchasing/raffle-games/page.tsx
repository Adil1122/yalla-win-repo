...
export default function UserPurchaseRaffleGame() {  
   ...
   const getInvoices = async() => {
      try {
         var user = localStorage.getItem('yalla_logged_in_user') !== null ? JSON.parse(localStorage.getItem('yalla_logged_in_user') + '') : '';
         let response = await fetch("/api/user/current-game-purchasing?user_id=" + user._id + "&skip=" + skip + '&limit=' + recordsPerPage + "&platform_type=web", {
            method: "GET",
         });
         const content = await response.json();
         console.log(content)
            ...
            <table className="w-full">
               <thead>
                  <tr className="bg-white">
                     ...
                     <th scope="col" className="px-4 py-5 lg:px-8 text-sm lg:text-size-1 whitespace-nowrap font-medium text-center text-darkone">Ticket Number</th> 
                     ...
                     invoices.map((invoice: any) => (
                        ...
                        <tr key={invoice._id}>
                           ...
                           <td key={invoice._id + '1'} className="whitespace-nowrap px-4 lg:py-5 lg:px-8 text-sm lg:text-size-1 text-white text-center">
                              {invoice.productInInvoice[0].name}
                              <img key={invoice._id + '3'} className="w-[70px] mx-auto mt-2" src={invoice.productInInvoice[0].image} alt="" />
                           </td>
                           ...
                           <td key={invoice._id + '4'} className="whitespace-nowrap px-4 lg:py-5 lg:px-8 text-sm lg:text-size-1 text-white text-center">{invoice.ticketInInvoice[0].ticket_number}</td>
                           ...
                           <td key={invoice._id + '8'} className="whitespace-nowrap px-4 lg:py-5 lg:px-8 text-sm lg:text-size-1 text-white text-center">{formatDate(invoice.draw_date)}</td>
